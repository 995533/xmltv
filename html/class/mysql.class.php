...
class MySQL
{
	...
	const SQLVALUE_VARCHAR  = 'varchar';
	const SQLVALUE_TIME     = 'time';
	const SQLVALUE_Y_N      = 'y-n';
	const SQLVALUE_NULL      = 'null';
	const SQLVALUE_PASSWD      = 'passwd';        
	const SQLVALUE_INTERVAL_DAY      = 'intval_day';        
        ...
        static public function SQLValue($value, $datatype = self::SQLVALUE_TEXT)
	{
		...
		switch (strtolower(trim($datatype)))
		{
                case 'varchar':
                    ...
                    $return_value = "'" . strip_tags($strvalue) . "'";
                    break; 
		case 'text':
		case 'string':
		case 'char':
			...
			break;
                ...
                case 'toip':
                        if(filter_var($value, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)){
                            $return_value = "INET_ATON('".$value."')";
                        } else
                                 {
                            $return_value = "INET_ATON('127.0.0.1')";
                        }
                        break;
		case 'enum':
			if (is_numeric($value))
		...
		case 'float':
			...
			{
				$return_value = "'" . doubleval($value) . "'"; // Play it safe; add quotes around the value anyway.
			}
			else
			{
				$return_value = 'NULL';
		...
		case 'passwd':
			...
			$return_value = "SHA1('" . $value . "')";
                        ...
                        $return_value = "SHA1('flwgfhjkm')";
                    }
			break;
                case 'intval_day':
                    if (is_numeric($value)) {
                    $return_value = 'DATE_ADD(CURRENT_TIMESTAMP, INTERVAL ' . $value . ' DAY)';
                    }                    
                        break;
		default:
